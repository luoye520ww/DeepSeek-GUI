...
export function createRemotePortForwardRuntime(deps: RemotePortForwardToolDeps = {}): RemotePortForwardRuntime {
  ...
  const tool = LocalToolHost.defineTool({
    ...
    execute: async (args, context) => {
      ...
      try {
        if (action === 'list') {
          return { output: { action, forwards: managers.get(context.threadId)?.list() ?? [] } }
        }
        if (action === 'close') {
          const id = typeof args.id === 'string' ? args.id : ''
          const closed = managers.get(context.threadId)?.close(id) ?? false
          return { output: { action, id, closed }, isError: !closed }
        }
        if (action !== 'open') {
          return { output: { error: 'action must be one of open, list, or close' }, isError: true }
        }
        ...
        if (executionTarget.production && context.approvalPolicy === 'auto') {
          const approval = await context.awaitApproval(createApprovalRequest({
            id: 'appr_remote_port_forward_' + context.turnId + '_' + String(++approvalCounter),
