...
const RUN_MODE_OPTIONS: Array<{ value: RemoteRunMode; label: string }> = [
  { value: 'observe', label: 'Observe: read-only inspection' },
  { value: 'develop', label: 'Develop: edit project files' },
  { value: 'operations', label: 'Operations: inspect services' },
  { value: 'deploy', label: 'Deploy: deployment commands' }
]
  ...
  return (
    <section className="w-full rounded-2xl border border-ds-border-muted bg-ds-card/80 p-3 text-xs shadow-sm">
      <div className="flex flex-wrap items-center gap-3">
        <span className={labelClass}>Run location</span>
        ...
        {isRemote ? (
          <span className="text-[11px] text-ds-muted">
            Applies only to the next new thread; existing threads keep their current target.
          </span>
        ) : null}
      </div>
